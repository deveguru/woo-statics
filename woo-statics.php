...
class WooStatics {
 ...
 .woo-preview-card { display: flex; flex-direction: column; justify-content: center; align-items: center; background: white; padding: 25px; border-radius: 10px; box-shadow: 0 5px 15px rgba(0,0,0,0.1); border-bottom: 4px solid #00CED1; }
 .woo-preview-card:last-child { border-bottom-color: #10B7EF; }
 ...
 if (!$user_data) {
 return '<div class="woo-statics-container"><p>کاربر یافت نشد.</p></div>';
 }
 
 $purchase_data = $this->get_user_purchase_data($user_id, $atts['limit']);
 
 if (empty($purchase_data['products'])) {
 return $this->render_no_data($user_data->display_name);
 }
 ...
 $completed_orders = count(wc_get_orders(array('customer_id' => $user_id, 'status' => 'wc-completed', 'limit' => -1)));
 $processing_orders = count(wc_get_orders(array('customer_id' => $user_id, 'status' => 'wc-processing', 'limit' => -1)));
 
 ob_start();
 ?>
 <div class="woo-preview-container">
 <div class="woo-preview-card">
 <div class="woo-preview-number"><?php echo number_format($completed_orders); ?></div>
 <div class="woo-preview-label">سفارشات تکمیل شده</div>
 </div>
 <div class="woo-preview-card">
 <div class="woo-preview-number"><?php echo number_format($processing_orders); ?></div>
 ...
 $monthly_data[$month_key]['total'] += floatval($order->get_total());
 ...
 $monthly_data[$month_key]['items'] += $quantity;
 ...
 $products[$product_id]['orders_count']++;
 $products[$product_id]['average_price'] = $products[$product_id]['total'] / $products[$product_id]['quantity'];
 }
 }
 ...
 <div class="woo-login-notice">
 <h3>برای مشاهده آمار خرید خود وارد شوید</h3>
 <p>جهت مشاهده اطلاعات خریدهای خود، لطفاً ابتدا وارد حساب کاربری خود شوید.</p>
 <a href="' . esc_url($login_url) . '" class="woo-login-button">ورود به حساب کاربری</a>
 </div>
 </div>';
 }
 ...
 return '
 <div class="woo-statics-container">
 <div class="woo-statics-header">
 <h2 class="woo-statics-title">آمار خرید ' . esc_html($user_name) . '</h2>
 </div>
 <div class="woo-no-data">
 <div class="woo-no-data-icon">🛒</div>
 <h3>هنوز خریدی انجام نداده‌اید</h3>
 <p>پس از انجام اولین خرید، آمار شما در اینجا نمایش داده خواهد شد.</p>
 </div>
 </div>';
 }
 ...
 <div class="woo-statics-container">
 <div class="woo-statics-header">
 <h2 class="woo-statics-title">آمار خرید <?php echo esc_html($user_name); ?></h2>
 <p class="woo-statics-subtitle">تحلیل کامل خریدهای شما از فروشگاه</p>
 </div>
 
 <div class="woo-stats-grid">
 <div class="woo-stat-card">
 <div class="woo-stat-number"><?php echo number_format($data['total_spent']); ?></div>
 <div class="woo-stat-label">مجموع خرید (تومان)</div>
 </div>
 <div class="woo-stat-card">
 <div class="woo-stat-number"><?php echo number_format($data['total_orders']); ?></div>
 <div class="woo-stat-label">تعداد سفارشات</div>
 </div>
 <div class="woo-stat-card">
 <div class="woo-stat-number"><?php echo number_format($data['total_items']); ?></div>
 <div class="woo-stat-label">تعداد کل محصولات</div>
 </div>
 <div class="woo-stat-card">
 <div class="woo-stat-number"><?php echo number_format($data['average_order_value']); ?></div>
 <div class="woo-stat-label">میانگین هر سفارش (تومان)</div>
 </div>
 </div>
 
 <div class="woo-charts-container">
 <div class="woo-chart-wrapper">
 <h3 class="woo-chart-title">مبلغ خرید محصولات</h3>
 <div class="woo-chart-canvas">
 <canvas id="productAmountChart<?php echo $unique_id; ?>"></canvas>
 </div>
 </div>
 
 <div class="woo-chart-wrapper">
 <h3 class="woo-chart-title">تعداد محصولات خریداری شده</h3>
 <div class="woo-chart-canvas">
 <canvas id="productQuantityChart<?php echo $unique_id; ?>"></canvas>
 </div>
 </div>
 </div>
 
 <?php if (!empty($data['monthly_data'])): ?>
 <div class="woo-chart-wrapper" style="margin-bottom: 30px;">
 <h3 class="woo-chart-title">روند خرید ماهانه</h3>
 <div class="woo-chart-canvas">
 <canvas id="monthlyTrendChart<?php echo $unique_id; ?>"></canvas>
 </div>
 </div>
 <?php endif; ?>
 
 <div class="woo-products-table">
 <div class="woo-table-header">
 لیست کامل محصولات خریداری شده
 </div>
 ...
 <div class="woo-product-stat">
 <span class="woo-product-stat-number"><?php echo number_format($product['quantity']); ?></span>
 <span class="woo-product-stat-label">تعداد</span>
 </div>
 <div class="woo-product-stat">
 <span class="woo-product-stat-number"><?php echo number_format($product['total']); ?></span>
 <span class="woo-product-stat-label">مجموع (تومان)</span>
 </div>
 <div class="woo-product-stat">
 <span class="woo-product-stat-number"><?php echo number_format($product['average_price']); ?></span>
 <span class="woo-product-stat-label">میانگین قیمت</span>
 </div>
 </div>
 </div>
 <?php endforeach; ?>
 ...
 const colorPalette = ['#00CED1', '#606060', '#000000', '#EEEEEE', '#10B7EF'];
 const productNames = <?php echo json_encode(array_values(array_column($data['products'], 'name'))); ?>;
 const productTotals = <?php echo json_encode(array_values(array_column($data['products'], 'total'))); ?>;
 const productQuantities = <?php echo json_encode(array_values(array_column($data['products'], 'quantity'))); ?>;
 ...
 type: 'doughnut',
 data: {
 labels: productNames.slice(0, isMobile ? 6 : 10),
 datasets: [{
 data: productTotals.slice(0, isMobile ? 6 : 10),
 backgroundColor: colorPalette,
 borderColor: '#ffffff',
 borderWidth: isMobile ? 2 : 3,
 hoverBorderWidth: isMobile ? 3 : 5
 }]
 },
 options: {
 ...responsiveOptions,
 ...
 datasets: [{
 label: 'تعداد',
 data: productQuantities.slice(0, isMobile ? 6 : 10),
 ...
 <?php if (!empty($data['monthly_data'])): ?>
 const monthlyLabels = <?php echo json_encode(array_keys($data['monthly_data'])); ?>;
 const monthlyTotals = <?php echo json_encode(array_column($data['monthly_data'], 'total')); ?>;
 const monthlyOrders = <?php echo json_encode(array_column($data['monthly_data'], 'orders')); ?>;
 ...
 backgroundColor: colorPalette[4] + '20',
 borderWidth: isMobile ? 2 : 3,
 fill: false,
 tension: 0.4,
 pointRadius: isMobile ? 3 : 4,
...
function woo_statics_activate() {
 ...
 wp_die('این افزونه نیاز به نصب و فعال‌سازی WooCommerce دارد.');
 }
 flush_rewrite_rules();
}
